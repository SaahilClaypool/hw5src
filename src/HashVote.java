...
public class HashVote implements VoteData {
    ...
    private HashMap<String, Integer> getSumMap() {
        ...
        secondChoice.forEach((key, value) -> {
            totalPoints.put(key, 2 * totalPoints.get(key) + (2 * value));
        });
        thirdChoice.forEach((key, value) -> {
            totalPoints.put(key, totalPoints.get(key) + (1 * value));
        }); // TODO double check logic and test this
        return totalPoints;
    }
    ...
    public String getWinnerMostPoints() {
        ...
        return maxKey[0]; // TODO refactor out counting.
    }
